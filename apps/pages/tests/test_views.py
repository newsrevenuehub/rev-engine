...
class TestPageViewSet:
    ...
    def unexpected_user(self, request):
        return request.getfixturevalue(request.param)

    def test_retrieve_when_unauthorized_user(self, unexpected_user, expected_status, live_donation_page, api_client):
    ...
    def test_list_when_unauthorized_user(self, unexpected_user, api_client):
        ...
        assert api_client.get(reverse("donationpage-list")).status_code == status.HTTP_403_FORBIDDEN
...
class TestStyleViewSet:
    ...
    def test_list_when_unauthorized_user(self, unexpected_user, api_client):
        ...
        api_client.force_authenticate(unexpected_user)
        response = api_client.get(reverse("style-list"))
        assert response.status_code == status.HTTTP_403_FORBIDDEN
    ...
    def test_delete_when_unexpected_user(self, unexpected_user, style, api_client):
        api_client.force_authenticate(unexpected_user)
        assert api_client.delete(reverse("style-detail", args=(style.pk,))).status_code == status.HTTP_404_NOT_FOUND
...
class TestFontViewSet:
    ...
    def test_unpermitted_methods(self, user, method, font, api_client):
        if user:
            api_client.force_authenticate(user)
        assert (
            getattr(api_client, method)(reverse("font-detail", args=(font.id,))).status_code
            == status.HTTP_405_METHOD_NOT_ALLOWED
        )
