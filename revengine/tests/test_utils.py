...
MINIMALLY_VALID_GS_SERVICE_ACCOUNT = base64.b64encode(
    json.dumps(
        {
...
@pytest.mark.parametrize(
    "raw_value,expect_value",
    (
        ("", False),
        (None, False),
        (MINIMALLY_VALID_GS_SERVICE_ACCOUNT, True),
        (base64.b64encode(json.dumps({}).encode("utf-8")), False),
    ),
)
@pytest.mark.parametrize("raise_if_unset", (True, False))
def test___ensure_gs_credentials(raw_value, expect_value, raise_if_unset, settings):
    if not expect_value and raise_if_unset:
        with pytest.raises(ImproperlyConfigured):
            __ensure_gs_credentials(raw_value, raise_if_unset)
    else:
        val = __ensure_gs_credentials(raw_value, raise_if_unset)
        assert val if expect_value else not val
