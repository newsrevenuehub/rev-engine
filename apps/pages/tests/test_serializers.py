...
from apps.users.tests.factories import RoleAssignmentFactory, create_test_user
...
class DonationPageFullDetailSerializerTest(RevEngineApiAbstractTestCase):
    ...
    def test_create(self):
        ...
        # Serial int suffix specifically called for in DEV-2297.
        assert "my-new-page2" == page2.slug
        assert f"{rp.name}1" == page2.name  # Serial int suffix specifically called for in DEV-2297.

    def test_create_page_has_default_elements_when_none_provided(self):
        rp = self.page.revenue_program
        page = self._post_page(
            {
                # name is autogenerated from RevenueProgram.
...
class StyleListSerializerTest(APITestCase):
    def setUp(self):
        self.org = OrganizationFactory(plan_name=Plans.PLUS)
    ...
    def test_plan_style_limits_are_respected(self):
        self.org.plan_name = Plans.FREE
        self.org.save()
        assert self.org.plan.style_limit == 1
