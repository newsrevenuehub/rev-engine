...
class TestDonationPageFullDetailSerializer:
    ...
    def test_create_page_has_default_elements_when_none_provided(self, revenue_program, hub_admin_user):
        ...
        assert serialized.is_valid()
        page = serialized.save()
        assert {x["type"] for x in page.elements} == {x["type"] for x in get_default_page_elements()}
    ...
    def test_create_with_permitted_free_plan_page_elements(self, revenue_program, hub_admin_user):
        ...
        revenue_program.organization.save()
        with Path("apps/pages/tests/fixtures/free-plan-page-elements.json").open() as f:
    ...
    def test_create_with_permitted_free_plan_sidebar_elements(self, revenue_program, hub_admin_user):
        revenue_program.organization.plan_name = Plans.FREE
        revenue_program.organization.save()
        with Path("apps/pages/tests/fixtures/free-plan-sidebar-elements.json").open() as f:
            sidebar_elements = json.load(f)
        data = {
            "slug": "my-new-page",
            "revenue_program": revenue_program.pk,
    ...
    def test_create_with_unpermitted_free_plan_page_elements(self, revenue_program, hub_admin_user):
        revenue_program.organization.plan_name = Plans.FREE
        revenue_program.organization.save()
        with Path("apps/pages/tests/fixtures/plus-plan-page-elements.json").open() as f:
            page_elements = json.load(f)
        data = {
    ...
    def test_create_with_unpermitted_free_plan_sidebar_elements(self, revenue_program, hub_admin_user):
        revenue_program.organization.plan_name = Plans.FREE
        revenue_program.organization.save()
        with Path("apps/pages/tests/fixtures/plus-plan-sidebar-elements.json").open() as f:
            sidebar_elements = json.load(f)
        data = {
            # name is autogenerated from RevenueProgram.
            "slug": "my-new-page",
    ...
    def test_update_referenced_style_is_missing(self, live_donation_page):
        ...
        with pytest.raises(serializers.ValidationError, match="Could not find Style"):
    ...
    def test_serializer_not_broken_by_no_payment_provider(self, live_donation_page):
        ...
        live_donation_page.refresh_from_db()
        DonationPageFullDetailSerializer(instance=live_donation_page).data  # noqa: B018 useless .data access
    ...
    def test_unpermitted_page_elements_are_not_returned(self, live_donation_page):
        ...
        live_donation_page.revenue_program.organization.save()
        with Path("apps/pages/tests/fixtures/plus-plan-page-elements.json").open() as f:
            live_donation_page.page_elements = json.load(f)
    ...
    def test_unpermitted_sidebar_elements_are_not_returned(self, live_donation_page):
        ...
        live_donation_page.revenue_program.organization.save()
        with Path("apps/pages/tests/fixtures/plus-plan-sidebar-elements.json").open() as f:
            live_donation_page.sidebar_elements = json.load(f)
        live_donation_page.save()
    ...
    def test_validate_publish_limit(self, plan, existing_locale, attempted_locale, hub_admin_user):
        ...
        assert serializer.is_valid() is False
        assert str(serializer.errors["non_field_errors"][0]) == (
            f"Your organization has reached its limit of {org.plan.publish_limit} published"
    ...
    def test_validate_publish_limit_when_patching(self, plan, hub_admin_user):
        ...
        assert serializer.is_valid() is False
        assert str(serializer.errors["non_field_errors"][0]) == (
            f"Your organization has reached its limit of {org.plan.publish_limit}"
            f" published page{'' if org.plan.publish_limit == 1 else 's'}"
        )
    ...
    def test_validate_publish_limit_when_valid(self, plan, hub_admin_user):
        org = OrganizationFactory(plan_name=plan)
        rp = RevenueProgramFactory(organization=org)
        DonationPageFactory.create_batch(org.plan.publish_limit - 1, revenue_program=rp, published_date=timezone.now())
        request = APIRequestFactory().post("/")
        request.user = hub_admin_user
        serializer = DonationPageFullDetailSerializer(
            data={
    ...
    def test_validate_page_limit(self, plan, request_method, revenue_program):
