...
describe('Donation page', () => {
  describe('Routing', () => {
    ...
    it('should show live 404 page if API has problems retrieving page data', () => {
      cy.intercept('/api/v1/organizations/stripe_account_id/**', { fixture: 'stripe/org-account-id.json' });
      cy.intercept({ method: 'GET', pathname: getEndpoint(FULL_PAGE) }, { statusCode: 404 });
      cy.visit('/revenue-program-slug');
      cy.getByTestId('live-page-404').should('exist');
    });

    it('should show live 404 page if API has problems retrieving stripe account id for org', () => {
      cy.intercept('/api/v1/organizations/stripe_account_id/**', { statusCode: 500 });
