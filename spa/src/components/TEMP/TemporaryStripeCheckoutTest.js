...
function TemporaryStripeCheckoutTest() {
  const stripeRef = useRef(
    loadStripe(process.env.REACT_APP_HUB_STRIPE_API_PUB_KEY || "", {
      stripeAccount: JSON.parse(localStorage.getItem(LS_USER)).organization
        .stripe_account_id,
    })
  );
...
function CheckoutForm() {
  ...
  useEffect(() => {
    ...
    async function createPaymentIntent() {
      const orgSlug = "caktus-group"; // get me from url!
      const pageSlug = "give"; // get me from url!
      const contributorEmail = "michael.ashton+contributor@caktusgroup.com"; // get me from email input on page
      try {
