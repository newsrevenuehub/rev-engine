...
class TestPaymentIntentSucceeded:
    def test_when_contribution_found(self, payment_intent_succeeded_one_time_event, client, mocker):
        ...
        assert contribution.provider_payment_id == payment_intent_succeeded_one_time_event["data"]["object"]["id"]
        assert contribution.last_payment_date is not None
        assert contribution.status == ContributionStatus.PAID

    def test_when_contribution_not_found(self, payment_intent_succeeded_one_time_event, mocker, client):
...
@pytest.mark.django_db
def test_customer_subscription_untracked_event(client, customer_subscription_updated, mocker):
    ...
    mock_log_warning.assert_called_once_with(
        "StripeWebhookProcessor.route_request received unexpected event type %s", customer_subscription_updated["type"]
    )
...
def test_invoice_upcoming(interval, expect_reminder_email, client, invoice_upcoming, mocker):
