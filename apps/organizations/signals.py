...
def handle_rp_mailchimp_entity_setup(instance, sender, created, **kwargs) -> None:
...
def get_page_to_be_set_as_default(rp) -> DonationPage:
    ...
    if (count := rp.donation_pages.count()) == 0:
        return None
    elif count == 1:
        return rp.donation_pages.first()
    # Is it okay to only look at > 1
    elif count > 1:
        return (
            rp.donation_pages.filter(is_published=True).order_by("created").first()
            if rp.donation_pages.filter(is_published=True).exists()
            else rp.donation_pages.order_by("created").first()
        )


def handle_set_default_donation_page(instance: Organization, created: bool, **kwargs) -> None:
    """ """
    logger.debug("handle_set_default_donation_page called on rp %s", instance.id)
    # what do we want to do if assumption that there's only one RP is wrong?
    rp = instance.revenue_programs.first()
    if not rp:
        logger.warning("No RP found for organization %s", instance.id)
        return
    if rp and rp.default_donation_page:
        logger.debug("RP %s already has a default_donation_page", rp.id)
        return
    # Default donation page corresponds to the org’s only donation page that exists at the time of upgrade, if only one exists. If two pages exist, default donation page should correspond to the only page published at the time of upgrade. If two pages exist but neither is published, default donation page should correspond to the first page created for that Org/RP.
    if rp and not rp.default_donation_page:
        page = get_page_to_be_set_as_default(rp)
        if page:
...
def revenue_program_post_save(sender, instance: RevenueProgram, created: bool, **kwargs) -> None:
    """Update"""
    logger.debug("revenue_program_post_save called on rp %s", instance.id)
    handle_rp_mailchimp_entity_setup(instance, sender, created, **kwargs)


@receiver(post_save, sender=Organization)
def organization_post_save(sender, instance: Organization, created: bool, **kwargs) -> None:
    logger.debug("organization_post_save called on org %s", instance.id)
    handle_set_default_donation_page(instance, created, **kwargs)
