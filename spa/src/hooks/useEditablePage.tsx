...
export interface EditablePageContextResult {
   ...
   * also update `page` and set `pageChanges` to an empty object. See the
   * `updatePage` function returned by `useContributionPage` for the args this
   * function takes.
   */
  savePageChanges?: (screenshotBaseName?: string, elementToScreenshot?: HTMLElement) => Promise<void>;
  /**
...
const EditablePageContext = createContext<EditablePageContextResult>({
  deletePage: () => {
...
const EditablePageContextProviderPropTypes = {
  ...
  revenueProgramSlug: PropTypes.string.isRequired
};
...
export function EditablePageContextProvider(props: InferProps<typeof EditablePageContextProviderPropTypes>) {
  const { children, pageSlug, revenueProgramSlug } = props;
  const { deletePage, error, isError, isLoading, page, updatePage } = useContributionPage(revenueProgramSlug, pageSlug);
  const [pageChanges, setPageChanges] = useState<EditablePageContextResult['pageChanges']>({});
  const savePage = useCallback(
    (screenshotBaseName?: string, elementToScreenshot?: HTMLElement) => {
      if (!updatePage) {
      ...
      return updatePage(pageChanges, screenshotBaseName, elementToScreenshot);
    },
  ...
  if (updatePage) {
    context.savePageChanges = savePage;
  }
